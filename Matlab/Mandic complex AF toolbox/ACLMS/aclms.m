...
WLMS = zeros(N,1); % weight vector of standard input vector
AWLMS = zeros(N,1); % weight vector of conjugate input vector 
eALMS = zeros(1,L); % error
...
for i = 1:L
    ...
    conjinput = conj(filterinput);
    filteroutput(i) = transpose(filterinput(:,i)) * WLMS + transpose(conjinput(:,i)) * AWLMS;% filter output
    eALMS(i) = x(1,i+M) - filteroutput(i);
    WLMS = WLMS + mu * eALMS(i) * conj(filterinput(:,i));% weight update of standard input vector
    AWLMS = AWLMS + mu * eALMS(i) * conj(conjinput(:,i));% weight update of conjugate input vector
end
y = filteroutput;
